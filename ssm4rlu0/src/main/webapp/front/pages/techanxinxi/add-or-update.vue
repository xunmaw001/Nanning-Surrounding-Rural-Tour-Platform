<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}' class="title">特产名称</view>
				<input :style='{"boxShadow":"0 0 4rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(23, 152, 242, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"110rpx"}' :disabled="ro.techanmingcheng" v-model="ruleForm.techanmingcheng" placeholder="特产名称"></input>
			</view>
				...
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}' class="title">特产类型</view>
				<picker @change="techanleixingChange" :value="techanleixingIndex" :range="techanleixingOptions">
					<view :style='{"boxShadow":"0 0 4rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(23, 152, 242, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input">{{ruleForm.techanleixing?ruleForm.techanleixing:"请选择特产类型"}}</view>
				</picker>
			</view>
				...
				<view class="right-input" :style='{textAlign:"left"}' style="padding:0">
					...
					<image :style='{"width":"88rpx","boxShadow":"0 0 16rpx rgba(0,0,0,.3)","borderRadius":"100%","textAlign":"left","height":"88rpx"}' class="avator" v-else src="../../static/gen/upload.png" mode="aspectFill"></image>
												...
												<view :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"#ccc","margin":"0 0 20rpx 0","borderWidth":"0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group select">
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}' class="title">生产日期</view>
			...
			<!-- 否 -->
				...
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}' class="title">特产详情</view>
				<textarea :style='{"boxShadow":"0 0 4rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(23, 152, 242, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"280rpx"}' v-model="ruleForm.techanxiangqing" placeholder="特产详情"></textarea>
			</view>
		...
		async onLoad(options) {
			...
			this.user = res.data;
						...
						// ss读取
																		...
																		// 下拉框
			res = await this.$api.option(`techanleixing`,`techanleixing`,{});
			this.techanleixingOptions = res.data;
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`techanxinxi`, this.ruleForm.id);
		...
		methods: {
			...
			// 多级联动参数
				...
				this.$forceUpdate();
															...
															// 下拉变化
			techanleixingChange(e) {
				this.techanleixingIndex = e.target.value
				this.ruleForm.techanleixing = this.techanleixingOptions[this.techanleixingIndex]
			},
			...
			async onSubmitTap() {
																																																																																																																																																																																																																																																																																																																																																																if((!this.ruleForm.techanmingcheng)){
					this.$utils.msg(`特产名称不能为空`);
					return
				}
																																												if((!this.ruleForm.techanleixing)){
					this.$utils.msg(`特产类型不能为空`);
					return
				}
					...
					return
																																...
																																if(this.ruleForm.id){
					await this.$api.update(`techanxinxi`, this.ruleForm);
				}else{
					await this.$api.add(`techanxinxi`, this.ruleForm);
				}
